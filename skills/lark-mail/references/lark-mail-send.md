...
# 分别发送（保存为草稿）：每个收件人收到的邮件 To 只含自己
lark-cli mail +send --to alice@example.com,bob@example.com --subject '通知' --body '<p>请查收</p>' --send-separately

# 分别发送 + 立即发送：跨多个收件人逐一投递，相互不可见
lark-cli mail +send --to alice@example.com,bob@example.com --subject '通知' --body '<p>请查收</p>' --send-separately --confirm-send
```
...
| `--send-separately` | 否 | 标记草稿为「分别发送」：发送时每个 To/Cc 收件人只看见自己。仅在 EML 里写 `X-Lms-Send-Separately: 1` 头，服务端按收件人列表拆发投递。可与 `--cc`/`--bcc`/`--attach`/`--inline`/`--plain-text`/`--template-id`/`--request-receipt`/`--signature-id`/`--priority`/`--confirm-send`/`--send-time` 叠加。总收件人数为 1 时会打 warning（不拒绝） |
| `--dry-run` | 否 | 仅打印请求，不执行 |
...
`--send-separately` 与 `--bcc` 都能"对外隐藏部分收件人"，但语义不同：

| 维度 | `--send-separately` | `--bcc` |
|---|---|---|
| 投递逻辑 | 服务端按 To/Cc 列表逐人拆发；每人收到一份独立副本 | 单次投递，To/Cc + Bcc 共用同一封邮件 |
| 收件人看到的 To/Cc | 只看见自己（其它 To/Cc 在该副本里被剥离） | 看到完整的 To/Cc；Bcc 不可见 |
| 是否隐藏对方存在 | ✅ To/Cc 之间互相不可见 | ❌ To/Cc 互相可见；仅 Bcc 隐藏 |
| 典型场景 | 群发周报 / 通知 / 邀约，避免暴露收件人列表 | 抄送审计 / 自我留档 |

两者可以叠加使用：`--send-separately --bcc audit@example.com` 时，audit 拿到含原始 To/Cc 的整封邮件，其它 To/Cc 仍按"分别发送"语义拆开。
