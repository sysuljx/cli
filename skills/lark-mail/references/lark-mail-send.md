...
# 定时发送：2 小时后发送（需 --confirm-send）
lark-cli mail +send --to alice@example.com --subject '周报' \
  --body '<p>本周进展如下...</p>' --send-after 2h --confirm-send
...
| `--send-time <unix_ts>` | 否 | 定时发送的绝对时间（Unix 秒时间戳）。为空或 0 时立即发送。必须至少为当前时间 5 分钟后。需配合 `--confirm-send` 使用 |
| `--send-after <duration>` | 否 | 定时发送的相对时间（如 `30m`、`2h`、`1d`）。CLI 内部转换为绝对 Unix 秒。必须至少 5 分钟后。需配合 `--confirm-send` 使用 |
