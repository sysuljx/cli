...
| [`+draft-send`](references/lark-mail-compose.md) | Send an existing draft, optionally with scheduled send time. Use `--send-time` (Unix timestamp) or `--send-after` (relative duration like `30m`, `2h`) to schedule. |
| [`+cancel-scheduled-send`](references/lark-mail-compose.md) | Cancel a scheduled draft send. The message must be in SCHEDULED state. Returns the message to DRAFT state. Requires `--mailbox` and `--message-id`. |
...
### user_mailbox.drafts
  ...
  - `get` — 获取草稿详情
  - `list` — 拉取草稿列表
  - `send` — 发送草稿。支持可选参数 `send_time`（Unix 秒时间戳）实现定时发送，为空或 0 时立即发送
  - `cancel_scheduled_send` — 取消定时发送。将处于 `SCHEDULED` 状态的邮件退回 `DRAFT` 状态。需要 `message_id`（messageBizID）
  - `update` — 更新草稿
