...
- **定时发送（Scheduled Send）**：草稿可以设置定时发送，邮件进入 `SCHEDULED` 状态，在指定时间自动发送。可用 `--send-time`（Unix 时间戳）或 `--send-after`（相对时间，如 `30m`、`2h`、`1d`）指定发送时间。定时发送需至少距离当前 5 分钟。发送前可用 `+cancel-scheduled-send` 取消，将邮件退回草稿状态。
...
| [`+draft-send`](references/lark-mail-draft-send.md) | Send an existing draft by draft ID. Use --send-time or --send-after to schedule the send for a specific time. |
| [`+cancel-scheduled-send`](references/lark-mail-cancel-scheduled-send.md) | Cancel a scheduled email that has not been sent yet. The message will be moved back to drafts. |
...
### user_mailbox.drafts
  ...
  - `get` — 获取草稿详情
  - `list` — 拉取草稿列表
  - `send` — 发送草稿（支持 `send_time` 参数实现定时发送）
  - `cancel_scheduled_send` — 取消定时发送，将邮件退回草稿状态
  - `update` — 更新草稿
